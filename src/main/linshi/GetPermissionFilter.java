...
public class GetPermissionFilter implements HelperFilter {
    ...
    CacheWrapper.NoExceptionGuavaCache<String, PermissionDO> GET_PERMISSION_CACHE =
            new CacheWrapper.NoExceptionGuavaCache<>(builder -> builder.expireAfterWrite(10, TimeUnit.SECONDS));

    @Override
    public boolean run(RequestContext context) {

        Long menuId = getMenuId(context);
        // 设置请求的菜单ID
        context.setMenuId(menuId);

        String key = context.getRequestKey();

        PermissionDO permission = GET_PERMISSION_CACHE.get(menuId+"|"+key,()-> permissionService.selectPermissionByRequest(key));
