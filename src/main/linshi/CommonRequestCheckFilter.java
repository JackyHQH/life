...
import org.srm.common.util.CacheWrapper;
...
public class CommonRequestCheckFilter implements HelperFilter {
    ...
    CacheWrapper.NoExceptionGuavaCache<String, List<Long>> REQUEST_CHECK_CACHE =
            new CacheWrapper.NoExceptionGuavaCache<>(builder -> builder.expireAfterWrite(10, TimeUnit.SECONDS));
    ...
    public boolean run(RequestContext context) {
        ...
        String finalMemberType = memberType;
        List<Long> finalRoleIds = roleIds;
        String finalSourceType = sourceType;
        sourceIds = REQUEST_CHECK_CACHE.get(
                Stream.of(memberId,memberType,tenantId,roleIds.stream().map(Object::toString).collect(Collectors.joining("-")),permissionCode,sourceType,checkCurrentRole)
                        .map(String::valueOf).collect(Collectors.joining("|")), ()->{
                    PermissionCheckDTO queryDTO = new PermissionCheckDTO(finalMemberId, finalMemberType, tenantId, finalRoleIds,
                            finalPermissionCode, finalSourceType, checkCurrentRole);
                    return permissionPlusMapper.selectSourceIdsByMemberAndRole(queryDTO);
                });
